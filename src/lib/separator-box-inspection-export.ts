...
import { type SeparatorBoxInspection } from './separator-box-inspection'
...
export type SeparatorBoxExportPosto = {
  nome: string
  cnpj: string
...
function drawFooter(ctx: PdfContext) {
  ...
  ctx.page.drawText('Teu Posto - Vistoria da caixa separadora', {
    x: MARGIN_X,
    y: 24,
    size: 8,
    font: ctx.font,
...
export function buildSeparatorBoxInspectionPdfFileName(
  posto: SeparatorBoxExportPosto,
  inspection: SeparatorBoxInspection,
) {
  const date = inspection.inspected_at.slice(0, 10)
  return `Caixa-Separadora-${slugify(posto.nome) || 'posto'}-${date}.pdf`
}

export function buildSeparatorBoxInspectionBulkPdfFileName(posto: SeparatorBoxExportPosto) {
  const date = new Date().toISOString().slice(0, 10)
  return `Caixa-Separadora-${slugify(posto.nome) || 'posto'}-${date}.pdf`
}

export async function generateSeparatorBoxInspectionPrintPdf(
  posto: SeparatorBoxExportPosto,
  inspections: SeparatorBoxInspection[],
): Promise<Uint8Array> {
  ...
  if (!inspections.length) {
    ...
    ctx.page.drawText('Nenhuma vistoria disponivel para exportacao.', {
      x: MARGIN_X,
      y: ctx.y,
      size: 12,
      font: fontBold,
  ...
  for (const inspection of inspections) {
    ...
    ctx.page.drawText(sanitize('Vistoria da Caixa Separadora'), {
      x: MARGIN_X,
      y: ctx.y,
      size: 15,
      font: fontBold,
    ...
    drawKeyValue(ctx, 'Executado por', inspection.operator_full_name || '-')
    ...
    for (const section of photoSections) {
      ...
      if (section.image) {
        ctx.y -= 2
        drawEmbeddedImage(ctx, section.image, CONTENT_WIDTH, 180)
      } else {
    ...
    if (signature || inspection.operator_full_name) {
      ...
      drawKeyValue(ctx, 'Assinado por', inspection.operator_full_name || '-')
      if (signature) {
        ctx.y -= 2
...
export function downloadSeparatorBoxInspectionPdf(bytes: Uint8Array, fileName: string) {
